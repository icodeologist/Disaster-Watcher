...
func CreateReport(c *gin.Context) {
	...
	reportCreated := models.ReportCreatedResponse{
		ID:          userReport.UserId,
		CreatedBy:   userReport.User.UserName,
		Category:    string(userReport.Category),
		Description: userReport.Description,
		Location:    userReport.Location,
		Status:      "Unverified",
	}

	fmt.Println("USER cached Location : ", userReport.ISLocationCached)

	// Scoring algo --> scores > 1 -> verified
	//
...
func GetAllReportsByUserID(c *gin.Context) {
	...
	if err := database.DB.Where("user_id=?", userId).Find(&reportsPostedByUser).Error; err != nil {
